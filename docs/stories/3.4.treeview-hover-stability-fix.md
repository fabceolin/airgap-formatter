...
- [x] Task 1: Investigate hover flickering root cause (AC: 1, 2, 3, 4)
  - [x] Analyze current hover state handling in delegate
  - [x] Check if MouseArea propagation causes re-render
  - [x] Test if hoverEnabled on nested MouseAreas conflicts with parent
  - [x] Document root cause

- [ ] Task 2: Implement hover stability fix (AC: 1, 2, 3, 4, 5, 6, 7, 8)
  - [x] Implement fix based on investigation findings
  - [x] Possible solutions to evaluate:
    - Use HoverHandler instead of MouseArea for hover detection (ATTEMPTED - FAILED)
...
|------|--------|-------------|
| qt/qml/JsonTreeView.qml | Modified | Replaced MouseArea with HoverHandler+TapHandler for copy buttons |

### Completion Notes
- Root cause identified: Nested MouseAreas with hoverEnabled causing hover event propagation conflicts with parent delegate's containsMouse state
- **Work in progress** - initial fix attempt did not resolve the flickering
...
#### Entry #1: HoverHandler+TapHandler Attempt (2026-01-21)
**Approach:** Replaced MouseArea with HoverHandler+TapHandler for copy buttons
**Changes Made:**
- Lines 248-255: Replaced `MouseArea { id: copyValueMouse; hoverEnabled: true; ... }` with `HoverHandler { id: copyValueHover }` + `TapHandler { onTapped: ... }`
- Lines 278-285: Replaced `MouseArea { id: copyPathMouse; hoverEnabled: true; ... }` with `HoverHandler { id: copyPathHover }` + `TapHandler { onTapped: ... }`
- Updated button color bindings from `copyValueMouse.containsMouse` to `copyValueHover.hovered`
- Updated ToolTip visible bindings from `copyValueMouse.containsMouse` to `copyValueHover.hovered`

**Result:** FAILED - Hover flickering persists
**Hypothesis for failure:**
- HoverHandler may still affect parent MouseArea's containsMouse state in Qt 6 WASM
- The root cause may not be the inner hover detection but something else (delegate recycling, row highlight logic, or TreeView's own hover handling)

**Next approaches to try:**
1. Approach B: Add `propagateComposedEvents: false` to inner MouseAreas (revert to MouseArea)
2. Approach C: Track `anyButtonHovered` property to stabilize row background
3. Investigate if TreeView's `reuseItems: true` (line 35) causes delegate state issues
4. Check if the parent mouseArea (line 79-100) needs `preventStealing: true`
...
| 2026-01-21 | 1.1 | Attempted fix using HoverHandler+TapHandler pattern - FAILED | James (Dev Agent) |
