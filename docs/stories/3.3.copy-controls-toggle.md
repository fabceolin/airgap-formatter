...
- [x] Task 1: Add hover copy icons to TreeView delegate (AC: 1, 14, 15)
  - [x] Add copy icon components to TreeItemDelegate
  - [x] Implement hover state detection
  - [x] Fade in/out animation for icons
  - [x] Position icons at right edge of row

- [x] Task 2: Implement Copy Value functionality (AC: 2, 18, 19, 20)
  - [x] Add `copyValue(row)` method to JsonTreeView
  - [x] For primitives: copy raw value
  - [x] For containers: serialize subtree to JSON string via QJsonTreeModel.serializeNode()
  - [x] Call JsonBridge clipboard method
  - [x] Show visual feedback on success (toast notification)

- [x] Task 3: Implement Copy Path functionality (AC: 3, 18, 19, 20)
  - [x] Add `copyPath(path)` method to JsonTreeView
  - [x] Get jsonPath from model role
  - [x] Copy to clipboard
  - [x] Show visual feedback

- [x] Task 4: Add context menu (AC: 4)
  - [x] Create Menu component for tree items
  - [x] Add "Copy Value" menu item
  - [x] Add "Copy Path" menu item
  - [x] Wire up right-click handler
  - [x] Position menu at cursor (using popup())

- [x] Task 5: Add Expand/Collapse All controls (AC: 6, 7, 8)
  - [x] Add "Expand All" button to Toolbar
  - [x] Add "Collapse All" button to Toolbar
  - [x] Wire to JsonTreeView.expandAll() / collapseAll()
  - [x] Add keyboard shortcuts (Ctrl+E, Ctrl+Shift+E)
  - [x] Group buttons visually

- [x] Task 6: Implement view toggle (AC: 9, 10, 11, 12, 13, 16)
  - [x] Add toggle button to Toolbar
  - [x] Create viewMode property in Main.qml ("tree" | "text")
  - [x] Conditionally show JsonTreeView or OutputPane
  - [x] Default to "tree" mode
  - [x] Update button label based on mode

- [x] Task 7: Update Toolbar.qml layout (AC: 5, 16, 17)
  - [x] "Copy Output" button already present (copies formatted JSON)
  - [x] Add view toggle button
  - [x] Add expand/collapse button group
  - [x] Ensure responsive layout with separators
...
```
qt/qml/
├── Main.qml              # View mode state (modify)
├── Toolbar.qml           # Add new buttons (modify)
├── OutputPane.qml        # Existing text view (unchanged)
├── JsonTreeView.qml      # Add copy methods, context menu (modify)
└── Theme.qml             # May need icon colors (check)

qt/
├── jsonbridge.cpp        # Clipboard methods (may modify)
└── jsonbridge.h
```
...
Row {
    ...
    Rectangle {
        ...
        Text {
            ...
            text: "\uf0c5"  // clipboard icon (FontAwesome) or use "📋"
            font.pixelSize: 12
...
|------|--------|-------------|
| qt/qml/Toolbar.qml | Modified | Added view toggle, expand/collapse buttons, new signals |
| qt/qml/Main.qml | Modified | Connected toolbar signals to functionality |
| qt/qml/JsonTreeView.qml | Created (Story 3.2) | Includes copy functionality, context menu, toast feedback |

### Completion Notes
- Copy icons appear on hover with [V] for value and [P] for path
- Context menu on right-click with Copy Value, Copy Path, Expand All, Collapse All
- Toast notification ("Copied!") appears for 1.5 seconds on copy success
- View toggle button shows "Tree" or "Text" label
- Expand/Collapse buttons disabled (opacity 0.5) when in text mode
- Keyboard shortcuts: Ctrl+E, Ctrl+Shift+E, Ctrl+T all implemented
- Uses existing JsonBridge.copyToClipboard() which uses WASM clipboard API
...
| 2026-01-21 | 1.1 | Implemented Tasks 1-7; Task 8 pending testing | James (Dev Agent) |
